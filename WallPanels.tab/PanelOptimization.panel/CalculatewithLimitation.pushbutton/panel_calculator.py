...
class OptimizationStrategy(object):
    def __init__(self, prioritize_coverage=True, allow_vertical_stacking=True,
                 ...
                 panel_orientation="vertical", allow_panel_cuts=False):
        ...
        # If True, panels may be trimmed at priority zone boundaries.
        # If False, only whole panels are placed; leftover space is left empty.
        self.allow_panel_cuts = bool(allow_panel_cuts)
    ...
    def to_dict(self):
        ...
        return {
            ...
            "optimization_strategy": {
                ...
                "allow_panel_cuts": os.allow_panel_cuts
            }
        }
    ...
    def from_dict(cls, data):
        ...
        return cls(
            ...
            optimization_strategy=OptimizationStrategy(
                ...
                os_.get("allow_panel_cuts", False)
            )
        )
...
# =============================================================================
# SECTION: PRIORITY ZONE HELPERS
# =============================================================================

def _clip_regions_to_zone(regions, zone):
    """Intersect existing wall regions with a priority zone rectangle."""
    clipped = []
    for r in regions:
        cx1 = max(r['x_start'], zone['x_start'])
        cx2 = min(r['x_end'],   zone['x_end'])
        cy1 = max(r['y_start'], zone['y_start'])
        cy2 = min(r['y_end'],   zone['y_end'])
        if cx2 > cx1 and cy2 > cy1:
            clipped.append(dict(r, x_start=cx1, x_end=cx2, y_start=cy1, y_end=cy2))
    return clipped


def _fill_region_band_priority(region, y_start, y_end, panels, panel_counter,
                               region_openings, sorted_openings, constraints,
                               allow_cuts=False):
    """
    Fill one horizontal band of a priority zone region left-to-right.
    If allow_cuts=True, trims the last panel to the zone edge.
    Returns (updated_panel_counter, last_x_placed).
    """
    PANEL_WIDTH_MIN = constraints.min_width
    DIMENSION_INCREMENT = constraints.dimension_increment
    SHORT_MAX = constraints.short_max
    ...
    x_limit = region['x_end']
    x_cursor = max(0.0, region['x_start'])

    while x_cursor < x_limit:
        remaining = x_limit - x_cursor
        if remaining < PANEL_WIDTH_MIN:
            break

        # Lookahead for blocker openings
        future_blockers = [
        ...
        next_blocker = (min(future_blockers, key=lambda o: o.left_clearance_zone)
                        if future_blockers else None)

        hard_stop = x_limit
        target_is_opening = False

        if next_blocker:
            bridge_dist = next_blocker.right_clearance_zone - x_cursor
            can_bridge = (bridge_dist <= max_width_for_band
                          and bridge_dist >= PANEL_WIDTH_MIN
                          and next_blocker.right_clearance_zone <= x_limit)
            if can_bridge:
                panel_w = snap_down(bridge_dist, DIMENSION_INCREMENT)
                cand = Panel(x_cursor, y_start, panel_w, band_height,
                             "P{:02d}".format(panel_counter))
                cand.cutouts = calculate_panel_cutouts(cand, region_openings)
                panels.append(cand)
                panel_counter += 1
                x_cursor += (panel_w + spacing)
                continue
            elif next_blocker.left_clearance_zone < x_limit:
                hard_stop = next_blocker.left_clearance_zone
                target_is_opening = True

        dist = hard_stop - x_cursor
        if dist < PANEL_WIDTH_MIN:
            if target_is_opening:
                x_cursor = next_blocker.right_clearance_zone
            else:
                break
            continue
        ...
        panel_w = calculate_segment_layout(
            x_cursor, hard_stop,
            max_width_for_band, PANEL_WIDTH_MIN, DIMENSION_INCREMENT, spacing
        )

        # Clamp to zone boundary
        if x_cursor + panel_w > x_limit:
            if allow_cuts:
                trimmed = snap_down(x_limit - x_cursor, DIMENSION_INCREMENT)
                if trimmed >= PANEL_WIDTH_MIN:
                    panel_w = trimmed
                else:
                    break
            else:
        ...
        if target_is_opening and abs(x_cursor - (hard_stop + spacing)) < 1.0:
            x_cursor = next_blocker.right_clearance_zone

    return panel_counter, x_cursor

def place_panels_sequential(wall_width, wall_height, openings, constraints, orientation="vertical",
                            priority_zones=None, allow_panel_cuts=False):
    """
    ...
    # Fill user-selected zones first (in draw order) before normal placement.
    # filled_segments records covered (x1,x2,y1,y2) so Pass 2 skips them.
    # -----------------------------------------------------------------------
    filled_segments = []  # list of (x_start, x_end, y_start, y_end)

    if priority_zones:
        sorted_pzones = sorted(priority_zones, key=lambda z: z.get('order', 0))
        print(Ansi.CYAN + "[PRIORITY] Running Pass 1 for {} priority zone(s)".format(
            len(sorted_pzones)) + Ansi.RESET)
        print(Ansi.CYAN + "[PRIORITY] Available regions: {}".format(
            [(round(r['x_start'],1), round(r['x_end'],1)) for r in regions]) + Ansi.RESET)

        for pz in sorted_pzones:
            pz_x1 = float(pz['x_start'])
            pz_x2 = float(pz['x_end'])
            pz_y1 = float(pz['y_start'])
            pz_y2 = float(pz['y_end'])
            print(Ansi.CYAN + "[PRIORITY] Zone: x={:.1f}-{:.1f}in  y={:.1f}-{:.1f}in".format(
                pz_x1, pz_x2, pz_y1, pz_y2) + Ansi.RESET)

            clipped = _clip_regions_to_zone(regions, {
                'x_start': pz_x1, 'x_end': pz_x2,
                'y_start': pz_y1, 'y_end': pz_y2
            })
            print(Ansi.CYAN + "[PRIORITY] Clipped to: {}".format(
                [(round(r['x_start'],1), round(r['x_end'],1)) for r in clipped]) + Ansi.RESET)
                ...
                if horizontal_mode:
                    ...
                    while cy < cr['y_end']:
                        ...
                        if bh < PANEL_HEIGHT_MIN:
                            break
                        panel_counter, _ = _fill_region_band_priority(
                            cr, cy, cy + bh,
                            panels, panel_counter,
                            cr['openings'], sorted_openings,
                            constraints, allow_cuts=allow_panel_cuts
                        )
                        filled_segments.append((cr['x_start'], cr['x_end'], cy, cy + bh))
                        cy += bh
                else:
                    panel_counter, _ = _fill_region_band_priority(
                        cr, cr['y_start'], cr['y_end'],
                        panels, panel_counter,
                        cr['openings'], sorted_openings,
                        constraints, allow_cuts=allow_panel_cuts
                    )
                    filled_segments.append((cr['x_start'], cr['x_end'],
                                            cr['y_start'], cr['y_end']))

        print(Ansi.CYAN + "[PRIORITY] Pass 1 complete. {} panels placed.".format(
            len(panels)) + Ansi.RESET)

    # -----------------------------------------------------------------------
    # Helpers for Pass 2: skip over already-filled segments
    # -----------------------------------------------------------------------
    def _skip_filled(x, y_start, y_end):
        """Advance x past any filled segment that contains x."""
        changed = True
        while changed:
            changed = False
            for (sx1, sx2, sy1, sy2) in filled_segments:
                if sy2 <= y_start or sy1 >= y_end:
                    continue
                if sx1 <= x < sx2:
                    x = sx2 + spacing
                    changed = True
    ...
    def _filled_stop_ahead(x, y_start, y_end, region_x_end):
        """Return the x of the nearest upcoming filled segment (or region_x_end)."""
        stop = region_x_end
        for (sx1, sx2, sy1, sy2) in filled_segments:
            if sy2 <= y_start or sy1 >= y_end:
                continue
    ...
    for region in regions:
        ...
        for y_start, y_end in bands:
            ...
            max_width_for_band = SHORT_MAX if band_height > SHORT_MAX else LONG_MAX
            x_cursor = max(0.0, region['x_start'])
            # Skip past any priority-filled segments at band start
            x_cursor = _skip_filled(x_cursor, y_start, y_end)
            ...
            while x_cursor < region['x_end']:
                ...
                # Hard stop = nearest of: region end, next opening, next filled segment
                hard_stop_x = _filled_stop_ahead(x_cursor, y_start, y_end, region['x_end'])
                target_is_opening = False
                target_is_filled = (hard_stop_x < region['x_end'])

                if next_opening:
                    bridge_dist = next_opening.right_clearance_zone - x_cursor
                    can_bridge = (
                        bridge_dist <= max_width_for_band
                        and bridge_dist >= PANEL_WIDTH_MIN
                        and next_opening.right_clearance_zone <= hard_stop_x
                    )
                    if can_bridge:
                        panel_w = snap_down(bridge_dist, DIMENSION_INCREMENT)
                ...
                # Skip over any priority-filled segments mid-band
                x_cursor = _skip_filled(x_cursor, y_start, y_end)
    ...
    for opening in openings:
        ...
        if inter_right > inter_left and inter_top > inter_bottom:
            ...
            cutout_info = {
                ...
                # Clearances read by placement script _get_clearances()
                "jamb_clr_in": float(opening.clearances.rough_jamb),
                "head_clr_in": float(opening.clearances.rough_header),
                "sill_clr_in": float(opening.clearances.rough_sill)
            }
            cutouts.append(cutout_info)

    return cutouts

def process_wall(wall_id, wall_width, wall_height, openings, priority_zones=None):
    ...
    allow_cuts = bool(getattr(ACTIVE_CONFIG.optimization_strategy, 'allow_panel_cuts', False))

    panels = place_panels_sequential(
        wall_width, wall_height, openings,
        ACTIVE_CONFIG.panel_constraints, orientation,
        priority_zones=priority_zones,
        allow_panel_cuts=allow_cuts
    )
...
def process_all_walls(walls_rows, openings_rows, output_dir,
                      ...
                      priority_zones=None):
    """
    priority_zones : dict  { wall_id (str): [zone_dict, ...] }
        Each zone_dict: { x_start, x_end, y_start, y_end, order }
        Pass None (default) for no priority zones -- identical to previous behaviour.
    """
    global ACTIVE_CONFIG
    ...
    for wall_row in walls_rows:
        ...
        wall_priority_zones = (priority_zones or {}).get(wall_id, None)
        panel_records = process_wall(wall_id, wall_width, wall_height, openings,
                                     priority_zones=wall_priority_zones)
...
def edit_panel_dimensions(config):
    ...
    print("  {}OK Panel dimensions updated{}".format(Ansi.GREEN, Ansi.RESET))
    return config
...
def edit_clearances(config):
    ...
    print("  {}OK Clearances updated{}".format(Ansi.GREEN, Ansi.RESET))
    return config
...
def create_custom_config():
    ...
    if orient_choice == "2":
        ...
        config.project_name = "Custom Configuration"
        config.optimization_strategy.panel_orientation = "horizontal"
        print("  {}OK Starting with horizontal preset{}".format(Ansi.GREEN, Ansi.RESET))
    else:
        config.optimization_strategy.panel_orientation = "vertical"
        print("  {}OK Starting with vertical preset{}".format(Ansi.GREEN, Ansi.RESET))
