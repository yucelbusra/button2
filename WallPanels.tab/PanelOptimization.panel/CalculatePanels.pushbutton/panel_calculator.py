...
def panel_overlaps_clearance(panel, openings, constraints, allow_intentional=False):
    ...
    for opening in openings:
        # Cutout openings are intentionally bridged — never block on them
        if is_cutout_opening(opening, constraints): continue
        # allow_intentional means we are filling a gap that is known to be inside
        # a blocker's clearance zone on purpose (above/below storefronts), so skip
        # the clearance check for blocker openings in that case.
...
def fill_vertical_gap(region_x_start, region_x_end, gap_y_start, gap_y_end,
    ...
    if is_storefront:
        # Above/below a blocking storefront: fill only the storefront's own x span
        panel_x_start = region_x_start + spacing
        panel_x_end   = region_x_end - spacing
    else:
        # Above/below a regular blocker (tall door etc.): fill the full region width
        # The opening's clearance zone is already enforced by the main placement pass;
        # here we just need panels that sit beside the opening vertically.
...
def calculate_segment_layout(start_x, target_x, max_w, min_w, inc, spacing):
    # target_x is the clearance zone boundary — we must stop BEFORE it (leave spacing gap)
    total_dist = target_x - start_x - spacing
    if total_dist < min_w: return total_dist   # caller will handle too-small case
        ...
        if horizontal_mode:
            ...
            while cy < wall_height:
                ...
                if bh >= PANEL_HEIGHT_MIN:
                    ...
                    cy += bh + spacing   # FIX: include spacing between horizontal bands
                else: break
        else:
        ...
        for y_start, y_end in bands:
            ...
            while x_cursor < region['x_end']:
                ...
                if target_is_opening and abs(x_cursor - (hard_stop_x + spacing)) < 1.0:
                    x_cursor = next_opening.right_clearance_zone + spacing
...
def calculate_panel_cutouts(panel, openings):
    ...
    for opening in openings:
        ...
        # Cutout box = rough opening clearance only (what gets physically cut)
        ...
        if inter_right > inter_left and inter_top > inter_bottom:
            ...
            # (used by placement script as UNIT WIDTH/HEIGHT — the actual door/window size)
            raw_x = opening.x - p_left   # left edge of raw opening from panel left
            raw_y = opening.y - p_bottom  # bottom of raw opening from panel bottom

            cutout_info = {
                "id":           opening.id,
                "type":         opening.type,
                # Cutout box (includes rough clearance — defines the physical void shape)
                "x_in":         float(cutout_x),
                "y_in":         float(cutout_y),
...
def process_wall(wall_id, wall_width, wall_height, openings):
    ...
    if ACTIVE_CONFIG is None:
        ...
        ACTIVE_CONFIG = presets.get("vertical")  # FIX: was wrongly defaulting to "horizontal"
...
def process_all_walls(walls_rows, openings_rows, output_dir,
    ...
    # Always read all clearances from ACTIVE_CONFIG so they stay in sync
    ...
    # FIX Bug 4: ensure output directory exists BEFORE any file writes
    if not os.path.exists(output_dir):
        try:
            os.makedirs(output_dir)
        except Exception as e:
            print(Ansi.RED + "[ERROR] Could not create output dir: {}".format(e) + Ansi.RESET)
            return None, None
