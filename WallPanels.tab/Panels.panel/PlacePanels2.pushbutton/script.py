...
CHANGE vs. original: Family origin moved from bottom-LEFT-corner to bottom-CENTER.
Only one thing changes in the code: insertion X walks to (x_in + width/2)
instead of x_in, so the panel left edge still lands at x_in while rotations
pivot correctly around the panel center.
"""
...
# -----------------------------------------------------------------------
# VOID PARAMETERS  (exact names verified from family param dump)
# -----------------------------------------------------------------------
...
# Family adds these to UNIT size via its own formulas.
# -----------------------------------------------------------------------
CLEARANCE_BY_TYPE = {
...
# Minimum void size to avoid family constraint violations
...
def compute_panel_base_point(wall, panel, rotation_deg=0.0, extra_z_offset_in=0.0):
    ...
    x_in  = left edge of panel from wall start (inches) — from CSV
    y_in  = bottom of panel from wall base (inches) — from CSV

    Because the origin is now at bottom-CENTER (not bottom-left corner):
      - Walk x_in PLUS half the panel width along the wall so the left
        edge lands at x_in and the origin sits at the horizontal center.
      - Z stays at wall_base + y_in (origin is at panel BOTTOM, not mid-height).
      - Depth shifts to structural core center as before.
    """
    ...
    x_ft      = _feet(x_in)
    y_ft      = _feet(y_in)
    half_w_ft = _feet(width_in / 2.0)  # offset so left edge = x_in

    if x_ref == "start":
        pt_xy = vis_left + (wall_dir * (x_ft + half_w_ft))
    else:
        pt_xy = vis_right - (wall_dir * (x_ft + half_w_ft))
    ...
    # Z = wall base + panel bottom (origin is at bottom of panel, not mid-height)
    base_z = wall_base_z + y_ft + _feet(extra_z_offset_in)
    ...
    # Depth: move from wall location line to structural core center.
    # No panel-thickness correction needed — origin is on the panel FACE.
...
def _resolve_param(inst, param_name):
    """Try exact name and with/without '(default)' suffix, on instance then type."""
    variants = [param_name]
...
def set_void_parameters_for_cutouts(inst, panel_data):
    ...
    if num >= 1:
        c = cutouts[0]
        # Use raw opening size (no clearance) — family formulas add clearance on top
        ...
        # x_in / y_in are LOCAL to panel (0,0 = panel bottom-left corner)
        ...
        # Park void off-screen — X=panel_width, Y=panel_height keeps it safely outside panel bounds
        print("  [VOID1] OFF | parking void off-screen")
        ...
        _set_param(inst, V1_Y_OFFSET,    panel_h_ft,  "Void 1 Y Offset")
    ...
    if num >= 2:
        ...
        print("  [VOID2] OFF | parking void off-screen")
        ...
        _set_param(inst, V2_Y_OFFSET,    panel_h_ft,  "Void 2 Y Offset")
...
def place_panel_family(wall, panel, symbol, extra_z_offset_in=0.0, is_cutout=False):
    ...
    # Size params — cast to float safely (CSV values are strings)
    try:
        ...
        print("  [WARN] set_size_parameters failed for {0}: {1}".format(
...
def create_panel_as_direct_shape(wall, panel):
    try:
        ...
        half_w_ft = w_ft / 2.0
        thk       = 1.0 / 12.0
        # pt is bottom-center — shift left by half_w to get the bottom-left corner
...
def main():
    ...
    # Activate symbol inside transaction (Activate() modifies the document)
