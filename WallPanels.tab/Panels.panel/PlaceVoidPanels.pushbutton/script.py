...
def place_panel_family(wall, panel, symbol, extra_z_offset_in=0.0, is_cutout=False):
    ...
    if not inst:
        try:
            ...
            if lvl:
                # IMPORTANT: NewFamilyInstance with a level host interprets Z
                # as relative to that level's elevation. Subtract the level
                # elevation so panels land at the correct absolute height.
